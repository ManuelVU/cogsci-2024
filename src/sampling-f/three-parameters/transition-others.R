...
transition_others <- function(state_now,
  ...
  for (i in 1:n_others) {
    if (state_now[i] == 0 & state_after[i] == 0) {
      ...
      transition_prob[1] <- transition_prob[1] * 
        (1 - logit(beta + kappa * relative_similarity[i, 1]))
      
      transition_prob[2] <- transition_prob[2] * 
        (1 - logit(beta + kappa * relative_similarity[i, 2]))
      
    }
    else {
      
      transition_prob[1] <- transition_prob[1] *
        logit(beta + kappa * relative_similarity[i, 1])
      
      transition_prob[2] <- transition_prob[2] * 
        logit(beta + kappa * relative_similarity[i, 2])
